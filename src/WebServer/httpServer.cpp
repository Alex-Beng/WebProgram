...
std::unordered_map<std::string, std::string> page2data;
...
void httpServer::run() {
    ...
    std::unordered_map<SOCKET, std::thread> threads;
    while (1) {
        ...
        // threads[client_socket] = 
        std::thread(httpServer::clientHandler, client_socket, buf_size).detach();
